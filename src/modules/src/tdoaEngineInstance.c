...
NO_DMA_CCM_SAFE_ZERO_INIT tdoaEngineState_t tdoaEngineState_t1;// = {.alternative_deck = true};
NO_DMA_CCM_SAFE_ZERO_INIT tdoaEngineState_t tdoaEngineState_t2;// = {.alternative_deck = false};
 ...
 */
STATS_CNT_RATE_LOG_ADD(stMiss, &tdoaEngineState_t1.stats.contextMissCount)

/**
 * @brief The clock correction factor for the anchor with the id selected by the tdoaEngine.logId parameter
 */
LOG_ADD(LOG_FLOAT, cc, &tdoaEngineState_t1.stats.clockCorrection)
 ...
 */
LOG_ADD(LOG_UINT16, tof, &tdoaEngineState_t1.stats.tof)
 ...
 * A is selected using the tdoaEngine.logId parameter and B is selected by tdoaEngine.logOthrId.
 */
LOG_ADD(LOG_FLOAT, tdoa, &tdoaEngineState_t1.stats.tdoa)
 ...
 */
PARAM_ADD_CORE(PARAM_UINT8, logId, &tdoaEngineState_t1.stats.newAnchorId)
/**
 * @brief Id of anchor used for logging, secondary anchor.
 */
PARAM_ADD_CORE(PARAM_UINT8, logOthrId, &tdoaEngineState_t1.stats.newRemoteAnchorId)
...
PARAM_ADD(PARAM_UINT8, matchAlgo, &tdoaEngineState_t1.matchingAlgorithm)
